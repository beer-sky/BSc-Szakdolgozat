...
#ifndef LEMON_RADIX_HEAP_4_H
#define LEMON_RADIX_HEAP_4_H
...
namespace lemon
{
    ...
    private:
        ...
        unsigned _nonempty = 0; // binary number,i_th bit is 1 if i_th bucket is nonempty

        std::vector<RadixItem> _data;
        std::vector<int> _boxes;  // first element of the box
        std::vector<int> _bounds; // box[i] containst elements from b[i]+1 to b_[i+1]
    ...
    public:
        ...
        RadixHeap4(ItemIntMap &map, int minimum = 0, int capacity = 0)
            : _iim(map)
        {
            ...
            _bounds.push_back(std::numeric_limits<int>::max()); // 2.147.483.647 = 2^(31)-1
    ...
    private:
        ...
        void remove(int index)
        {
            ...
            if (_boxes[_data[index].box] == -1)
                _nonempty &= ~(1 << _data[index].box);
        }
        ...
        {
            if (_bounds[_data[index].box + 1] > _data[index].prio)
                return;
            remove(index);
        ...
        int findFirst()
        {
            if (_nonempty == 0)
                return 0;
            return __builtin_ctz(_nonempty);
        }
    ...
    public:
        ...
        Item top() const
        {
            const_cast<RadixHeap4<ItemIntMap> &>(*this).moveDown();
