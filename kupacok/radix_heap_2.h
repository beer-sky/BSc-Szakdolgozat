...
#ifndef LEMON_RADIX_HEAP_2_H
#define LEMON_RADIX_HEAP_2_H
...
namespace lemon
{
    ...
    public:
        ...
        RadixHeap2(ItemIntMap &map, int minimum = 0, int capacity = 0)
            : _iim(map)
        {
    ...
    public:
        ...
        void push(const Item &i, const Prio &p)
        {
            ...
            // TODO felező kereséssel a végét.
            int edge_bucket = _boxes.size() - (32 - __builtin_clz(n)) + 1;
            if (p > _bounds[edge_bucket])
            {
                int start = _boxes.size() - 1;
                while (_bounds[start] + 1 > p)
                {
                    if (--start < 0)
                        throw PriorityUnderflowError();
                }
        ...
        Item top() const
        {
            const_cast<RadixHeap2<ItemIntMap> &>(*this).moveDown();
